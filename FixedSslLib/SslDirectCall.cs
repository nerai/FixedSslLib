	...
	internal unsafe static class SslDirectCall
	{
		public static void CloseNotify (SslStream sslStream)
		{
			...
			fixed (void* workArrayPtr = workArray) {
				...
				if (isServer) {
					status = (NativeApi.SecurityStatus) NativeApi.AcceptSecurityContext (
						ref credentialsHandleHandle,
						ref securityContextHandle,
						null,
						inflags,
						NativeApi.Endianness.Native,
					...
					status = (NativeApi.SecurityStatus) NativeApi.InitializeSecurityContextW (
						ref credentialsHandleHandle,
						ref securityContextHandle,
						null,
						inflags,
